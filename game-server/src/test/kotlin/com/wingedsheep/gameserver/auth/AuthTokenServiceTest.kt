...
class AuthTokenServiceTest : FunSpec({
    ...
    test("issues a token that verifies back to the same claims") {
        val svc = service()
        val claims = svc.verify(svc.issue(42L, "player@example.com"))
        claims.shouldNotBeNull()
        claims.uid shouldBe 42L
    ...
    test("rejects a token after it expires") {
        ...
        val token = svc.issue(1L, "p@example.com", now)
    ...
    test("rejects a token with a tampered payload") {
        val svc = service()
        val token = svc.issue(1L, "p@example.com")
    ...
    test("rejects a token signed with a different secret") {
        val token = service(secret = "secret-one").issue(1L, "p@example.com")
