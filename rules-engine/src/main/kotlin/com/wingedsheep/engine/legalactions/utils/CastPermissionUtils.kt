...
class CastPermissionUtils(
    ...
    fun getStaticGrantedAbilitiesWithGranter(
        ...
        // ability-granting static — not one printed on its card — confers the inner activated
        // ability to the matching group exactly like a printed GrantActivatedAbility. Example:
        // Roar of the Fifth People chapter II, "This Saga gains 'Creatures you control have
        // "{T}: Add {R}, {G}, or {W}."'" — the outer static lands in [GameState.grantedStaticAbilities]
        // (via GrantStaticAbilityEffect), and the granter is the permanent that gained it.
        for (grantedStatic in state.grantedStaticAbilities) {
            val grantAbility = grantedStatic.ability as? com.wingedsheep.sdk.scripting.GrantActivatedAbility
                ?: continue
            ...
            when (val scope = grantAbility.filter.scope) {
                ...
                is com.wingedsheep.sdk.scripting.filters.unified.Scope.AttachedTo -> {
                    val attachedTo = granter.get<com.wingedsheep.engine.state.components.battlefield.AttachedToComponent>()
                    if (attachedTo != null && attachedTo.targetId == entityId) {
